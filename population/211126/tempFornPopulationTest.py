...
localFileName = pd.read_csv('/Users/choejiyeon/Documents/project/KOCCA/python/population/집계구_단기체류외국인_파일명.csv')
...
for key2, fileName in localFileName.iterrows():
    ...
    localDf = csv.read_csv(
        '/Users/choejiyeon/Documents/project/KOCCA/99.기타/Data/kocca_데이터_수집_파일/서울생활인구/집계구 단위(단기체류 외국인)/{0}/{1}'.format(
    ...
    if len(local_row) < 24:
        for i in range(24):
            if local_row.loc[local_row['f1'] == i].empty:
                data = {'f0': int(filepath[15:23]), 'f1': i, 'f2': adstrd_code, 'f3': smgu_cd, 'f4': -1}
...
smgu_local_list.columns = ['STDR_DE_ID', 'TMZON_PD_SE', 'ADSTRD_CODE_SE', 'SMGU_CD', 'TEMP_FORN_TOT']

# 빠진 날짜 데이터 -1로 채우기
local_miss = pd.DataFrame()
for miss_dt in miss_dt_list:
    for i in range(24):
        data = {'STDR_DE_ID': int(miss_dt), 'TMZON_PD_SE': i, 'ADSTRD_CODE_SE': adstrd_code, 'SMGU_CD': smgu_cd, 'TEMP_FORN_TOT': -1}
...
smgu_local_list.to_csv('/Users/choejiyeon/Documents/project/KOCCA/96.DB/부가데이터/서울생활인구/test/{0}_단기외국인.csv'.format(smgu_cd), mode="w", header=True, index=False)  # 데이터프레임 csv 파일로 저장
