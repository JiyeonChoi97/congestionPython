...
localFileName = pd.read_csv('/Users/choejiyeon/Documents/project/KOCCA/python/population/집계구_장기체류외국인_파일명.csv')
...
smgu_local_list.columns = ['STDR_DE_ID', 'TMZON_PD_SE', 'ADSTRD_CODE_SE', 'SMGU_CD', 'LONG_FORN_TOT']

# 빠진 날짜 데이터 -1로 채우기
local_miss = pd.DataFrame()
for miss_dt in miss_dt_list:
    for i in range(24):
        data = {'STDR_DE_ID': int(miss_dt), 'TMZON_PD_SE': i, 'ADSTRD_CODE_SE': adstrd_code, 'SMGU_CD': smgu_cd, 'LONG_FORN_TOT': -1}
...
smgu_local_list.to_csv('/Users/choejiyeon/Documents/project/KOCCA/96.DB/부가데이터/서울생활인구/test/{0}_장기외국인.csv'.format(smgu_cd), mode="w", header=True, index=False)  # 데이터프레임 csv 파일로 저장
