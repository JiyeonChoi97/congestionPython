...
for key, smguCode in smguCd.iterrows():
    ...
    #
    # # 데이터 중복 제거
    # smgu_local_list.drop_duplicates(['f0', 'f1', 'f2', 'f3', 'f4'])
    # smgu_long_forn_list.drop_duplicates(['f0', 'f1', 'f2', 'f3', 'f4'])
    # smgu_temp_forn_list.drop_duplicates(['f0', 'f1', 'f2', 'f3', 'f4'])
    ...
    main_data.to_csv('/Users/choejiyeon/Documents/project/KOCCA/96.DB/부가데이터/서울생활인구/smgu/{0}.csv'.format(smgu_cd), mode="w", header=True, index=False)  # 데이터프레임 csv 파일로 저장
