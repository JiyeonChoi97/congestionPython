...
# 집계구 코드 csv 파일 읽기
smguCd = pd.read_csv('/Users/choejiyeon/Documents/project/KOCCA/python/population/집계구행정동코드-1.csv')
...
for date in dt_index:
    ...
    if os.path.exists('/Users/choejiyeon/Documents/project/KOCCA/99.기타/Data/kocca_데이터_수집_파일/서울생활인구/집계구 단위(내국인)/LOCAL_PEOPLE_{0}/LOCAL_PEOPLE_{1}.csv'.format(searchDate[:6], searchDate)):
        ...
        for key, smguCode in smguCd.iterrows():
            ...
            # print(adstrd_code)
            # for i in range(24):
            #     data = {'STDR_DE_ID': int(searchDate), 'TMZON_PD_SE': i, 'ADSTRD_CODE_SE': adstrd_code, 'SMGU_CD': smgu_cd,
            #             'NATIVE_TOT': -1, 'LONG_FORN_TOT': -1, 'TEMP_FORN_TOT': -1}
            #     local_miss = local_miss.append(data, ignore_index=True)
            # print(local_miss)
            STDR_DE_IDs = [int(searchDate) for i in range(24)]
            TMZON_PD_SEs = [i for i in range(24)]
            ADSTRD_CODE_SEs = [adstrd_code for i in range(24)]
            SMGU_CDs = [smgu_cd for i in range(24)]
            NAN= [-1 for i in range(24)]

            local_miss = pd.DataFrame({'STDR_DE_ID': STDR_DE_IDs, 'TMZON_PD_SE': TMZON_PD_SEs, 'ADSTRD_CODE_SE': ADSTRD_CODE_SEs, 'SMGU_CD': SMGU_CDs, 'NATIVE_TOT': NAN, 'LONG_FORN_TOT': NAN, 'TEMP_FORN_TOT': NAN})
            # '집계구.csv' 에 데이터 쓰기
            f = open(
                '/Users/choejiyeon/Documents/project/KOCCA/96.DB/부가데이터/서울생활인구/Data/{0}.csv'.format(smgu_cd), 'a', newline='')
            wr = csv.writer(f)
    ...
    for key, smguCode in smguCd.iterrows():
        ...
        main_data2 = main_data2.fillna(int(-1))
