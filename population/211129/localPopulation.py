...
def work(id, smgu_cd):
    ...
    for key2, fileName in localFileName.iterrows():
        ...
        local_row = localDf.loc[localDf['f3'] == int(smgu_cd)]  # 집계구 내국인 데이터에서 특정 값(집계구코드)을 가진 행 찾기
    ...
    for key2, fileName in longFornFileName.iterrows():
        ...
        long_forn_row = longFornDf.loc[
            longFornDf['f3'] == int(smgu_cd)]  # 집계구 장기체류 외국인 데이터에서 특정 값(집계구코드)을 가진 행 찾기
        # 집계구별 일별 데이터의 개수가 24개가 아닌경우 비어있는 시간대 데이터값 -1 채워주기
    ...
    print(id, " 장기체류 외국인 시작시간 : {0}, 종료시간 : {1}, 걸린시간 : {2}".format(start_time2, end_time2, elapsed_time2))
    ...
    for key2, fileName in tempFornFileName.iterrows():
        ...
        temp_forn_row = tempFornDf.loc[tempFornDf['f3'] == int(smgu_cd)]  # 집계구 단기체류 외국인 데이터에서 특정 값(집계구코드)을 가진 행 찾기
    ...
    print(id, " 단기체류 외국인 시작시간 : {0}, 종료시간 : {1}, 걸린시간 : {2}".format(start_time3, end_time3, elapsed_time3))
    ...
    main_data.columns = ['STDR_DE_ID', 'TMZON_PD_SE', 'ADSTRD_CODE_SE', 'SMGU_CD', 'NATIVE_TOT', 'LONG_FORN_TOT',
                         'TEMP_FORN_TOT']
    ...
    for miss_dt in miss_dt_list:
        for i in range(24):
            data = {'STDR_DE_ID': int(miss_dt), 'TMZON_PD_SE': i, 'ADSTRD_CODE_SE': adstrd_code, 'SMGU_CD': smgu_cd,
    ...
    main_data.to_csv(
        '/Users/choejiyeon/Documents/project/KOCCA/96.DB/부가데이터/서울생활인구/smgu/{0}.csv'.format(smgu_cd),
    ...
    print(smgu_cd, id,  " 시작시간 : {0}, 종료시간 : {1}, 걸린시간 : {2}".format(start_time4, end_time4, elapsed_time4))
...
if __name__ == "__main__":
    ...
    thList = []
    for i in range(50):
        th1 = Process(target=work, args=(i, smguCd.iloc[i]))
        thList.append(th1)
