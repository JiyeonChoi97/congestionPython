...
while (True):  # 반복실행
    ...
    for date in dt_list:
        for time in time_list:
            ...
            # print("poi_id : " + str(row[0]) + ", date : " + date + ", time : " + time + " , inNativeCnt : " + str(inNativeCnt) + " , nativeCnt : " + str(nativeCnt) + " , fornCnt : " + str(fornCnt) + " , totCnt : " + str(totCnt) + " , congestion : " + str(congestion))

    print("---------------" + row[1] + "추출 완료---------------")

congestion_df.to_csv('주데이터_관광객수_혼잡도.csv', mode="w", header=True, index=False)    # 데이터프레임 csv 파일로 저장
print("---------------주데이터_관광객수_혼잡도.csv 완료---------------")
